...
res.status(200).json(planets)
       ...
       res.status(400).json({msg:'errore nella ricerca degli elementi della tabella'})
    }
}
...
export const insertWithId = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ msg: `Pianeta aggiornato correttamente all'id: ${id}` });
    } catch (error) {
...
export const deletePlanetsById=async (req,res)=>{
    ...
    try {
        ...
        res.status(200).json({msg:`pianeta eliminato correttamente all'id ${id}`})
